...
struct IndexPage {
    ...
    TPrimaryKey pk{};
    long long positionPages{}; // Posición de la página en el archivo de datos pages[m+1]
};
...
void ISAM::insert(Record record) {
    ...
    ofstream createFile(indexfilename, ios::binary);
...
static void printIndexRecord(IndexPage &indexRecord) {
    std::visit([](auto&& arg){ cout << arg; }, indexRecord.pk); // Use std::visit correctly
    cout << " " << indexRecord.positionPages << endl;
}
...
void ISAM::loadCSV(const string& csv_dir) {
    ...
    for(int i = 0 ; i < len ; i++){
        ...
        indexRecord.positionPages = data.tellp();
        writeRecord(record, data, defaultpos);
        auto PrimaryKey = visit([](auto&& arg) { return arg.getKey(); }, record.csv);
        indexRecord.pk = PrimaryKey;
        index.write((char*)&indexRecord, sizeof(indexRecord));
    }
...
// MODIFICAR PARA QUE FUNCIONE CON STRINGS, auto pk = visit([](auto&& arg) { return arg.getKey(); }, record.csv);
// pk.index() == 0 -> int
// pk.index() == 1 -> float
// pk.index() == 2 -> string ESTE DEBEMOS MODIFICAR Y USAR strcmp
long long ISAM::find(const TPrimaryKey &pk, fstream &file) {
    long long low = 0;
    long long high = (getFileSize(file) / sizeof(IndexPage)) - 1;
    long long pos = 0;
    IndexPage indexRecord;
...
void ISAM::insert(Record record) {
    ...
    if (!data.is_open() || !index.is_open()) {
        if (!data.is_open()) cout << "Data file not open" << endl;
        if (!index.is_open()) cout << "Index file not open" << endl;
        return;
    }
    ...
    vector<IndexPage> rewrite;
    unsigned long defaultpos = 0;
    auto key = visit([](auto&& arg) { return arg.getKey(); }, record.csv);
    ...
    index.seekg(pos + sizeof(IndexPage));
...
optional<Record> ISAM::search(const TPrimaryKey& key){
    ifstream data(datafilename, ios::binary);
    ifstream index(indexfilename, ios::binary);
    IndexPage indexRecord;
    Record record;
...
vector<Record> ISAM::rangeSearch(const TPrimaryKey& begin, const TPrimaryKey& end){
    ifstream data(datafilename, ios::binary);
    ifstream index(indexfilename, ios::binary);
    IndexPage indexRecord;
    Record record;
...
bool ISAM::remove(const TPrimaryKey& key) {
    ifstream dataIn(datafilename, ios::binary);
    fstream index(indexfilename, ios::in | ios::out | ios::binary);
    if(!dataIn || !index) return false;
    IndexPage indexRecord;
    ...
    ofstream indexOut(indexfilename, ios::trunc | ios::binary);
    for(auto & rec : dataRewrite) {
        auto PrimaryKey = visit([](auto&& arg) { return arg.getKey(); }, rec.csv);
        indexRecord.pk = PrimaryKey;
        indexRecord.positionPages = dataOut.tellp();
        writeRecord(rec, dataOut, defaultpos);
        writeIndex(indexRecord, indexOut);
    }
    dataOut.close();
    indexOut.close();
    return true;
